<template>
  ...
  >
    <span
      >{{ t("yys.property.currentSelectionPrefix") }}{{ current.name }}</span
    >
    <el-form :model="shikigami" label-width="120px">
      ...
      <el-form-item :label="t('yys.property.skillRequired')">
        ...
        >
          <el-select
            v-for="(value, key, index) in shikigami.skillRequired"
            :placeholder="value"
            style="margin-bottom: 10px"
            @change="updateSkillRequired(key, $event)"
          >
...
watch(
  () => shikigami.value.skillRequiredMode,
  (newVal) => {
    if (newVal == "all") {
  ...
  (newVal) => {
    switch (newVal) {
      ...
      case "0": {
        ...
        shikigami.value.yuhun.property2 = [
          "Attack",
          "Defense",
          "Health",
          "Speed",
        ];
        shikigami.value.yuhun.property4 = [
          "Attack",
          "Defense",
          "Health",
          "ControlHit",
        ...
        shikigami.value.yuhun.property6 = [
          "Attack",
          "Defense",
          "Health",
          "Crit",
...
const updateYuhunSelect = (yuhun, operator) => {
  ...
  if (operator == "Update") {
    if (yuhunIndex.value >= 0)
      shikigami.value.yuhun.yuhunSetEffect[yuhunIndex.value] = JSON.parse(
        JSON.stringify(yuhun),
      );
    else
      shikigami.value.yuhun.yuhunSetEffect.push(
        JSON.parse(JSON.stringify(yuhun)),
      );
  }
  //Delete
