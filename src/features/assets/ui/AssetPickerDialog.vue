<template>
  <el-dialog v-model="show" :title="config.title">
    <span v-if="config.currentItem">
      {{ translate("selector.currentSelection") }}：{{
    ...
    >
      <el-input
        v-model="searchText"
        :placeholder="translate('selector.searchPlaceholder')"
...
import { buildSelectorItemKey } from "../selectorItemKey";
import {
  createCustomAssetFromFile,
  listCustomAssets,
  subscribeCustomAssetStore,
} from "../customAssetsRepository";
import type { GroupConfig, SelectorConfig } from "../model/selector";
...
const handleSelect = (item: any) => {
  ...
  const normalizedItem = {
    ...item,
    [field]: props.resolveAssetUrl(item?.[field]),
  };
...
const getItemImageUrl = (item: any) =>
  props.resolveAssetUrl(item?.[imageField.value]) as string;
