...
const customAssetStorage = createSafeStorage(CUSTOM_ASSET_STORAGE_KEY);
...
const readStore = (): CustomAssetStore => {
  ...
  const raw = customAssetStorage.read();
  if (!raw) {
    return {};
  }
  try {
    ...
    if (normalizedRaw !== raw) {
      customAssetStorage.write(normalizedRaw);
    }
    return normalized;
  } catch {
    // ignore
...
const writeStore = (store: CustomAssetStore) => {
  ...
  if (customAssetStorage.write(JSON.stringify(store))) {
    notifyStoreUpdated();
  }
};
