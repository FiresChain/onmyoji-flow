...
const nodeAppearanceStorage = createSafeStorage(NODE_CREATE_SIZE_STORAGE_KEY);
...
export const readNodeCreateSizeConfig = (): NodeCreateSizeConfig => {
  const raw = nodeAppearanceStorage.read();
  if (!raw) {
  ...
  try {
    ...
    if (normalizedRaw !== raw) {
      nodeAppearanceStorage.write(normalizedRaw);
    }
    return normalized;
  } catch {
    return cloneNodeCreateSizeConfig(DEFAULT_NODE_CREATE_SIZE_CONFIG);
...
export const writeNodeCreateSizeConfig = (
  ...
  nodeAppearanceStorage.write(JSON.stringify(normalized));
  return normalized;
};

export const resetNodeCreateSizeConfig = (): NodeCreateSizeConfig => {
  nodeAppearanceStorage.remove();
  return cloneNodeCreateSizeConfig(DEFAULT_NODE_CREATE_SIZE_CONFIG);
};
