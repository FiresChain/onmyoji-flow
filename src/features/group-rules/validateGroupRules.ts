...
import {
  ...
  evaluateCompiledRuleExpression,
  type CompiledRuleExpression,
} from "./expression";
import type { GroupRulesConfig } from "./model/types";
import {
  getDynamicGroupChildIds,
  normalizeDynamicGroupNode,
} from "./model/groupMeta";
import { readSharedGroupRulesConfig } from "./rulesRepository";
...
type CompiledExpressionResult =
  | { ok: true; expression: CompiledRuleExpression }
  | { ok: false; error: unknown };

const getCompiledExpression = (
  expression: string,
  cache: Map<string, CompiledExpressionResult>,
  ...
  let result: CompiledExpressionResult;
  try {
    result = { ok: true, expression: compileRuleExpression(expression) };
  } catch (error) {
...
const evaluateCondition = (
  ...
  const compiled = getCompiledExpression(expression, cache);
  if ("error" in compiled) {
    console.warn("[groupRules] 表达式执行失败:", expression, compiled.error);
    return false;
  }

  try {
    return !!evaluateCompiledRuleExpression(compiled.expression, scope);
  } catch (error) {
...
export const validateGraphGroupRules = (
  ...
  const normalizedGraphData = {
    ...
    nodes: normalizedGraph.nodes.map((node) =>
      normalizeAssetSelectorNode(normalizeDynamicGroupNode(node)),
    ),
  };
  ...
  const expressionCache = new Map<string, CompiledExpressionResult>();
  ...
  teams.forEach((team) => {
    ...
    effectiveConfig.expressionRules.forEach((rule) => {
      ...
      if (scopeKind === "team") {
        if (!evaluateCondition(rule.condition, teamScope, expressionCache)) {
          return;
        }
        warnings.push({
      ...
      team.shikigamiUnits.forEach((unit) => {
        ...
        if (
          !evaluateCondition(rule.condition, shikigamiScope, expressionCache)
        ) {
          return;
        }
        warnings.push({
