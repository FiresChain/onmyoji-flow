...
describe('useToolbarImportExportCommands', () => {
  ...
  it('handleExport and handlePreviewData keep deterministic counts across pre-threshold flush variant matrix with rebatch isolation', () => {
    const context = createContext();
    const stringifySpy = vi.spyOn(JSON, 'stringify');
    let expectedUpdateTabCount = 0;
    let expectedPreviewTriggerCount = 0;
    ...
    const flushVariantMatrix = [
      {
        firstBatch: ['preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['export', 'preview', 'export', 'preview', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['preview', 'export', 'preview'] as Array<'preview' | 'export'>,
      },
    ];

    flushVariantMatrix.forEach(({ firstBatch, secondBatch }) => {
      const firstBatchPreviewCount = countBatchActions(firstBatch, 'preview');
      const firstBatchExportCount = countBatchActions(firstBatch, 'export');
      triggerInterleavedBatch(firstBatch);
      expect(vi.getTimerCount()).toBe(firstBatch.length);
      ...
      vi.runOnlyPendingTimers();
      executedPreviewCount += firstBatchPreviewCount;
      executedExportCount += firstBatchExportCount;
      expect(vi.getTimerCount()).toBe(0);
      expect(stringifySpy).toHaveBeenCalledTimes(executedPreviewCount);
      expect(context.filesStore.exportData).toHaveBeenCalledTimes(executedExportCount);
      expect(context.filesStore.updateTab).toHaveBeenCalledTimes(expectedUpdateTabCount);

      const secondBatchPreviewCount = countBatchActions(secondBatch, 'preview');
      const secondBatchExportCount = countBatchActions(secondBatch, 'export');
      triggerInterleavedBatch(secondBatch);
      expect(vi.getTimerCount()).toBe(secondBatch.length);
      expect(stringifySpy).toHaveBeenCalledTimes(executedPreviewCount);
      expect(context.filesStore.exportData).toHaveBeenCalledTimes(executedExportCount);
      ...
      vi.advanceTimersByTime(1);
      executedPreviewCount += secondBatchPreviewCount;
      expect(vi.getTimerCount()).toBe(secondBatchExportCount);
      expect(stringifySpy).toHaveBeenCalledTimes(executedPreviewCount);
      expect(context.filesStore.exportData).toHaveBeenCalledTimes(executedExportCount);
      ...
      vi.advanceTimersByTime(1);
      executedExportCount += secondBatchExportCount;
      expect(vi.getTimerCount()).toBe(0);
      expect(stringifySpy).toHaveBeenCalledTimes(executedPreviewCount);
      expect(context.filesStore.exportData).toHaveBeenCalledTimes(executedExportCount);
      expect(context.filesStore.updateTab).toHaveBeenCalledTimes(expectedUpdateTabCount);
  ...
  it('handleExport and handlePreviewData keep timer determinism across repeated pre-threshold flush matrices with rebatch isolation', () => {
    const context = createContext();
    const stringifySpy = vi.spyOn(JSON, 'stringify');
    let expectedUpdateTabCount = 0;
    let expectedPreviewTriggerCount = 0;
    ...
    const preThresholdFlushVariants = [
      {
        firstBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'export'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['export', 'preview', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['preview', 'preview', 'export', 'export', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'preview', 'export', 'export'] as Array<'preview' | 'export'>,
      },
    ];

    preThresholdFlushVariants.forEach(({ firstBatch, secondBatch }) => {
      const firstBatchPreviewCount = countBatchActions(firstBatch, 'preview');
      const firstBatchExportCount = countBatchActions(firstBatch, 'export');
      triggerInterleavedBatch(firstBatch);
      expect(vi.getTimerCount()).toBe(firstBatch.length);
  ...
  it('handleExport and handlePreviewData keep deterministic counts across repeated pre-threshold flush loops with multi-rebatch isolation', () => {
    const context = createContext();
    const stringifySpy = vi.spyOn(JSON, 'stringify');
    let expectedUpdateTabCount = 0;
    let expectedPreviewTriggerCount = 0;
    ...
    const repeatedPreThresholdFlushMatrix = [
      {
        firstBatch: ['preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['export', 'preview', 'export'] as Array<'preview' | 'export'>,
        secondBatch: ['preview', 'preview', 'export'] as Array<'preview' | 'export'>,
        thirdBatch: ['export', 'preview', 'export', 'preview', 'preview'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['preview', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'export', 'export', 'preview'] as Array<'preview' | 'export'>,
      },
    ];

    repeatedPreThresholdFlushMatrix.forEach(({ firstBatch, secondBatch, thirdBatch }) => {
      const firstBatchPreviewCount = countBatchActions(firstBatch, 'preview');
      const firstBatchExportCount = countBatchActions(firstBatch, 'export');
      triggerInterleavedBatch(firstBatch);
      expect(vi.getTimerCount()).toBe(firstBatch.length);
      ...
      const thirdBatchPreviewCount = countBatchActions(thirdBatch, 'preview');
      const thirdBatchExportCount = countBatchActions(thirdBatch, 'export');
      triggerInterleavedBatch(thirdBatch);
      expect(vi.getTimerCount()).toBe(thirdBatch.length);
      expect(stringifySpy).toHaveBeenCalledTimes(executedPreviewCount);
      expect(context.filesStore.exportData).toHaveBeenCalledTimes(executedExportCount);
      expect(context.filesStore.updateTab).toHaveBeenCalledTimes(expectedUpdateTabCount);
      ...
      vi.advanceTimersByTime(1);
      executedPreviewCount += thirdBatchPreviewCount;
      expect(vi.getTimerCount()).toBe(thirdBatchExportCount);
      expect(stringifySpy).toHaveBeenCalledTimes(executedPreviewCount);
      expect(context.filesStore.exportData).toHaveBeenCalledTimes(executedExportCount);
      ...
      vi.advanceTimersByTime(1);
      executedExportCount += thirdBatchExportCount;
      expect(vi.getTimerCount()).toBe(0);
      expect(stringifySpy).toHaveBeenCalledTimes(executedPreviewCount);
      expect(context.filesStore.exportData).toHaveBeenCalledTimes(executedExportCount);
      expect(context.filesStore.updateTab).toHaveBeenCalledTimes(expectedUpdateTabCount);
  ...
  it('handleExport and handlePreviewData keep timer determinism under alternating pre-threshold flush loops and immediate rebatch isolation', () => {
    const context = createContext();
    const stringifySpy = vi.spyOn(JSON, 'stringify');
    let expectedUpdateTabCount = 0;
    let expectedPreviewTriggerCount = 0;
    ...
    const alternatingFlushRebatchMatrix = [
      {
        firstBatch: ['preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'export'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
      ...
      },
      {
        ...
        secondBatch: ['preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['export', 'preview', 'preview', 'export'] as Array<'preview' | 'export'>,
      },
    ];

    alternatingFlushRebatchMatrix.forEach(({ firstBatch, secondBatch, thirdBatch }) => {
  ...
  it('handleExport and handlePreviewData keep timer determinism under repeated pre-threshold flush and chained immediate rebatch isolation', () => {
    const context = createContext();
    const stringifySpy = vi.spyOn(JSON, 'stringify');
    let expectedUpdateTabCount = 0;
    let expectedPreviewTriggerCount = 0;
    ...
    const chainedImmediateRebatchMatrix = [
      {
        firstBatch: ['preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'export'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
    ...
    chainedImmediateRebatchMatrix.forEach(({ firstBatch, secondBatch, thirdBatch }) => {
      settleByPreThresholdFlush(firstBatch);
      expect(vi.getTimerCount()).toBe(0);
  ...
  it('handleExport and handlePreviewData reinforce timer determinism under dual-window hybrid flush and chained rebatch zero-residual loops', () => {
    const context = createContext();
    const stringifySpy = vi.spyOn(JSON, 'stringify');
    let expectedUpdateTabCount = 0;
    let expectedPreviewTriggerCount = 0;
    ...
    const dualWindowHybridFlushMatrix = [
      {
        firstBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'preview', 'export'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'export', 'export', 'preview', 'preview'] as Array<'preview' | 'export'>,
        fourthBatch: ['export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['export', 'preview', 'export'] as Array<'preview' | 'export'>,
        secondBatch: ['preview', 'preview', 'export', 'export'] as Array<'preview' | 'export'>,
        thirdBatch: ['export', 'preview', 'preview', 'export'] as Array<'preview' | 'export'>,
        fourthBatch: ['preview', 'export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['preview', 'preview', 'export'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
        fourthBatch: ['export', 'export', 'preview', 'preview'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['export', 'preview', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['preview', 'export', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['export', 'preview', 'export'] as Array<'preview' | 'export'>,
        fourthBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'export', 'preview', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'export', 'export', 'preview'] as Array<'preview' | 'export'>,
        fourthBatch: ['export', 'preview', 'preview', 'export'] as Array<'preview' | 'export'>,
      },
    ];

    dualWindowHybridFlushMatrix.forEach(({ firstBatch, secondBatch, thirdBatch, fourthBatch }) => {
      settleByPreThresholdFlush(firstBatch);
      expect(vi.getTimerCount()).toBe(0);
  ...
  it('handleExport and handlePreviewData reinforce timer determinism under alternating dual-window hybrid flush and chained immediate rebatch zero-residual loops', () => {
    const context = createContext();
    const stringifySpy = vi.spyOn(JSON, 'stringify');
    let expectedUpdateTabCount = 0;
    let expectedPreviewTriggerCount = 0;
    ...
    const settleByPreThresholdFlushWithZeroResidual = (batch: Array<'preview' | 'export'>) => {
      const batchPreviewCount = countBatchActions(batch, 'preview');
      const batchExportCount = countBatchActions(batch, 'export');
    ...
    const settleBySegmentedWindowWithNoEarlyTrigger = (batch: Array<'preview' | 'export'>) => {
      const batchPreviewCount = countBatchActions(batch, 'preview');
      const batchExportCount = countBatchActions(batch, 'export');
    ...
    const alternatingDualWindowHybridFlushMatrix = [
      {
        firstBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'preview', 'export', 'export'] as Array<'preview' | 'export'>,
        fourthBatch: ['export', 'preview', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['export', 'preview', 'preview', 'export'] as Array<'preview' | 'export'>,
        secondBatch: ['preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        fourthBatch: ['preview', 'export', 'export', 'preview'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['preview', 'export', 'export', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        fourthBatch: ['export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['export', 'preview', 'export'] as Array<'preview' | 'export'>,
    ...
    alternatingDualWindowHybridFlushMatrix.forEach(({ firstBatch, secondBatch, thirdBatch, fourthBatch }) => {
      settleByPreThresholdFlushWithZeroResidual(firstBatch);
      expect(vi.getTimerCount()).toBe(0);

      settleByPreThresholdFlushWithZeroResidual(secondBatch);
      expect(vi.getTimerCount()).toBe(0);

      settleBySegmentedWindowWithNoEarlyTrigger(thirdBatch);
      expect(vi.getTimerCount()).toBe(0);

      settleBySegmentedWindowWithNoEarlyTrigger(fourthBatch);
      expect(vi.getTimerCount()).toBe(0);
  ...
  it('handleExport and handlePreviewData reinforce timer determinism under alternating dual-window hybrid flush repeated pre-threshold loops and chained immediate rebatch zero-residual cycles', () => {
    const context = createContext();
    const stringifySpy = vi.spyOn(JSON, 'stringify');
    let expectedUpdateTabCount = 0;
    let expectedPreviewTriggerCount = 0;
    ...
    const settleByRepeatedPreThresholdFlush = (batch: Array<'preview' | 'export'>) => {
      const batchPreviewCount = countBatchActions(batch, 'preview');
      const batchExportCount = countBatchActions(batch, 'export');
    ...
    const alternatingDualWindowHybridFlushRepeatedLoopsMatrix = [
      {
        firstBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'preview', 'export', 'export'] as Array<'preview' | 'export'>,
      ...
      {
        firstBatch: ['preview', 'preview', 'export', 'export', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'export', 'preview', 'preview', 'export'] as Array<'preview' | 'export'>,
        fourthBatch: ['export', 'export', 'preview', 'preview'] as Array<'preview' | 'export'>,
      },
    ];

    alternatingDualWindowHybridFlushRepeatedLoopsMatrix.forEach(({ firstBatch, secondBatch, thirdBatch, fourthBatch }) => {
      settleByRepeatedPreThresholdFlush(firstBatch);
      expect(vi.getTimerCount()).toBe(0);
      vi.runOnlyPendingTimers();
      expect(vi.getTimerCount()).toBe(0);

      settleByRepeatedPreThresholdFlush(secondBatch);
      expect(vi.getTimerCount()).toBe(0);
      vi.runOnlyPendingTimers();
      expect(vi.getTimerCount()).toBe(0);
  ...
  it('handleExport and handlePreviewData reinforce timer determinism under alternating dual-window hybrid flush repeated pre-threshold loops and chained immediate rebatch zero-residual cycles v2', () => {
    const context = createContext();
    const stringifySpy = vi.spyOn(JSON, 'stringify');
    let expectedUpdateTabCount = 0;
    let expectedPreviewTriggerCount = 0;
    ...
    const assertZeroResidualTimers = () => {
      expect(vi.getTimerCount()).toBe(0);
      vi.runOnlyPendingTimers();
      expect(vi.getTimerCount()).toBe(0);
    };
    ...
    const alternatingDualWindowHybridFlushRepeatedLoopsMatrixV2 = [
      {
        firstBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'preview', 'export', 'export'] as Array<'preview' | 'export'>,
      ...
      },
      {
        ...
        secondBatch: ['preview', 'export', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
        fourthBatch: ['export', 'preview', 'preview', 'export', 'export'] as Array<'preview' | 'export'>,
      },
    ];

    alternatingDualWindowHybridFlushRepeatedLoopsMatrixV2.forEach(({ firstBatch, secondBatch, thirdBatch, fourthBatch }) => {
      settleByRepeatedPreThresholdFlush(firstBatch);
      assertZeroResidualTimers();
  ...
  it('handleExport and handlePreviewData reinforce timer determinism under alternating dual-window repeated pre-threshold flush multi-rebatch segmented windows and zero-residual timer idempotence cycles v3', () => {
    const context = createContext();
    const stringifySpy = vi.spyOn(JSON, 'stringify');
    let expectedUpdateTabCount = 0;
    let expectedPreviewTriggerCount = 0;
    ...
    const alternatingDualWindowRepeatedPreThresholdFlushMultiRebatchMatrixV3 = [
      {
        firstBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'preview', 'export', 'export'] as Array<'preview' | 'export'>,
        fourthBatch: ['export', 'preview', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        fifthBatch: ['preview', 'export', 'preview', 'export', 'export'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['export', 'preview', 'preview', 'export'] as Array<'preview' | 'export'>,
        secondBatch: ['preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        fourthBatch: ['preview', 'export', 'export', 'preview'] as Array<'preview' | 'export'>,
        fifthBatch: ['export', 'preview', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['preview', 'export', 'export', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        fourthBatch: ['export', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        fifthBatch: ['preview', 'preview', 'export', 'export', 'preview'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['export', 'preview', 'export'] as Array<'preview' | 'export'>,
        secondBatch: ['preview', 'preview', 'export', 'export'] as Array<'preview' | 'export'>,
        thirdBatch: ['export', 'preview', 'preview', 'export'] as Array<'preview' | 'export'>,
        fourthBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
        fifthBatch: ['export', 'preview', 'export', 'preview', 'preview'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['preview', 'preview', 'export', 'export', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['export', 'preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'export', 'preview', 'preview', 'export'] as Array<'preview' | 'export'>,
        fourthBatch: ['export', 'export', 'preview', 'preview'] as Array<'preview' | 'export'>,
        fifthBatch: ['preview', 'export', 'export', 'preview', 'preview'] as Array<'preview' | 'export'>,
      },
      {
        firstBatch: ['export', 'preview', 'preview', 'export', 'preview'] as Array<'preview' | 'export'>,
        secondBatch: ['preview', 'export', 'export', 'preview'] as Array<'preview' | 'export'>,
        thirdBatch: ['preview', 'export', 'preview', 'export'] as Array<'preview' | 'export'>,
        fourthBatch: ['export', 'preview', 'preview', 'export', 'export'] as Array<'preview' | 'export'>,
        fifthBatch: ['preview', 'export', 'preview', 'preview', 'export'] as Array<'preview' | 'export'>,
      },
    ];

    alternatingDualWindowRepeatedPreThresholdFlushMultiRebatchMatrixV3.forEach(({
      firstBatch,
      secondBatch,
      thirdBatch,
      fourthBatch,
      ...
      settleBySegmentedWindow(fifthBatch);
      assertZeroResidualTimers();
