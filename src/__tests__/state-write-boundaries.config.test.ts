...
const boundaryRuleSources = import.meta.glob('../../eslint-rules/*-boundary.js', {
  eager: true,
  import: 'default',
  query: '?raw'
...
const extractBoundaryIdentifier = (ruleSource: string) => {
  const match = ruleSource.match(/createStateWriteBoundaryOptions\((['"])([A-Za-z0-9_]+)\1\)/)
  return match?.[2] ?? null
}
  ...
  it('所有 *-boundary 规则都必须通过集中配置源声明 stateIdentifier', () => {
    ...
    const ruleBoundaryIdentifiers = Object.entries(boundaryRuleSources)
      .map(([filePath, ruleSource]) => {
        ...
        expect(boundaryIdentifier, `${filePath} must call createStateWriteBoundaryOptions(...)`).not.toBeNull()
