...
describe("multi-instance embed isolation regression", () => {
  ...
  it("isolates active file and file list between instance A/B", () => {
    ...
    expect(editorA.store.fileList.map((file) => file.id)).toEqual([
      "a-1",
      "a-2",
    ]);
    expect(editorB.store.fileList.map((file) => file.id)).toEqual([
      "b-1",
      "b-2",
    ]);
  ...
  it("isolates canvas switch/save/update writes between instance A/B", () => {
    ...
    expect(
      (editorA.store.getTab("a-1") as any)?.graphRawData?.nodes?.[0],
    ).toMatchObject({
      id: "lf-a-node",
      zIndex: 11,
    });
    expect(
      (editorB.store.getTab("b-1") as any)?.graphRawData?.nodes?.[0]?.id,
    ).toBe("b-source");
    ...
    expect(
      (editorB.store.getTab("b-1") as any)?.graphRawData?.nodes?.[0],
    ).toMatchObject({
      id: "lf-b-node",
      zIndex: 22,
    });
    expect(
      (editorA.store.getTab("a-1") as any)?.graphRawData?.nodes?.[0]?.id,
    ).toBe("lf-a-node");
  });
