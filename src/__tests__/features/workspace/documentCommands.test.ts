...
import { ref } from "vue";
import { useDocumentCommands } from "@/features/workspace/useDocumentCommands";
...
vi.mock("@/features/workspace/documentTransfer", async () => {
  const actual = await vi.importActual<
    typeof import("@/features/workspace/documentTransfer")
  >("@/features/workspace/documentTransfer");
  return {
    ...actual,
...
interface DocumentCommandTestContext {
  state: {
...
const createContext = (): DocumentCommandTestContext => {
  const state = {
  ...
  const filesStore: DocumentCommandTestContext["filesStore"] = {
    updateTab: vi.fn(),
...
describe("useDocumentCommands", () => {
  ...
  it("handleExport and handlePreviewData keep deterministic timer counts across repeated interleaved rounds", () => {
    ...
    interleavedRounds.forEach((round) => {
      round.forEach((action) => {
        ...
        expect(context.filesStore.updateTab).toHaveBeenCalledTimes(
          expectedUpdateTabCount,
        );
      });
    });
    ...
    vi.advanceTimersByTime(1);
    expect(context.filesStore.exportData).toHaveBeenCalledTimes(
      expectedExportTriggerCount,
    );
    expect(context.filesStore.updateTab).toHaveBeenCalledTimes(
      expectedUpdateTabCount,
    );
  ...
  it("handleExport and handlePreviewData keep segmented determinism with partial-flush rebatching and zero-residual timers", () => {
    ...
    const firstBatch: Array<"preview" | "export"> = [
      "preview",
      "export",
      "preview",
      "export",
    ...
    const secondBatch: Array<"preview" | "export"> = [
      "export",
      "preview",
      "preview",
      "export",
  ...
  it("handleExport and handlePreviewData keep deterministic counts under pre-threshold flush and rebatch isolation", () => {
    ...
    const previewCountAfterFirstBatchFlush = firstBatch.filter(
      (action) => action === "preview",
    ).length;
    const exportCountAfterFirstBatchFlush = firstBatch.filter(
      (action) => action === "export",
    ).length;

    vi.runOnlyPendingTimers();
    expect(vi.getTimerCount()).toBe(0);
    expect(stringifySpy).toHaveBeenCalledTimes(
      previewCountAfterFirstBatchFlush,
    );
    expect(context.filesStore.exportData).toHaveBeenCalledTimes(
      exportCountAfterFirstBatchFlush,
    );
  ...
  it("triggerJsonFileImport keeps dialog state semantics across repeated parse failures", () => {
    ...
    expect(context.showMessage).toHaveBeenNthCalledWith(
      1,
      "error",
      "文件格式错误",
    );
  ...
  it("handleTeamCodeImport keeps conversion-failure error behavior", async () => {
    ...
    vi.mocked(convertTeamCodeToRootDocument).mockRejectedValue(
      new Error("阵容码转换失败") as never,
    );
  ...
  it("handleTeamCodeQrImport keeps decode-fill-clear behavior", async () => {
    ...
    vi.mocked(decodeTeamCodeFromQrImage).mockResolvedValue(
      "#TA#QR_CODE" as never,
    );

    await context.commands.handleTeamCodeQrImport({
      target,
    } as unknown as Event);
  ...
  it("handleTeamCodeQrImport keeps decoding state recovery for async success/failure branches", async () => {
    const context = createContext();
    const successFile = new File(["qr-success"], "team-code-success.png", {
      type: "image/png",
    });
    const successTarget = {
      files: [successFile],
    ...
    vi.mocked(decodeTeamCodeFromQrImage).mockReturnValueOnce(
      successDeferred.promise as never,
    );

    const successPromise = context.commands.handleTeamCodeQrImport({
    ...
    const failureFile = new File(["qr-failure"], "team-code-failure.png", {
      type: "image/png",
    });
    const failureTarget = {
      files: [failureFile],
    ...
    vi.mocked(decodeTeamCodeFromQrImage).mockReturnValueOnce(
      failureDeferred.promise as never,
    );

    const failurePromise = context.commands.handleTeamCodeQrImport({
  ...
  it("handleTeamCodeQrImport keeps decode-failure error behavior", async () => {
    const context = createContext();
    const file = new File(["broken-qr"], "broken-qr.png", {
      type: "image/png",
    });
    const target = {
      files: [file],
    ...
    vi.mocked(decodeTeamCodeFromQrImage).mockRejectedValue(
      new Error("二维码识别失败") as never,
    );
