...
import { useRuleManager } from "@/features/group-rules/ui/useRuleManager";
import {
...
} from "@/features/group-rules/rulesRepository";
import { ElMessageBox } from "element-plus";

vi.mock("@/features/group-rules/rulesRepository", () => ({
...
interface RuleManagerTestContext {
  state: {
    showRuleManagerDialog: boolean;
  };
  showMessage: ReturnType<typeof vi.fn>;
  composable: ReturnType<typeof useRuleManager>;
}
...
const createContext = (): RuleManagerTestContext => {
  const state = {
    showRuleManagerDialog: false,
  };
  const showMessage = vi.fn();
  const composable = useRuleManager({
    state,
    showMessage,
  });
