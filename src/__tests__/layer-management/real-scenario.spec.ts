...
describe("图层管理真实场景测试", () => {
  ...
  it("场景2: 置顶操作（模拟右键菜单）", () => {
    ...
    const allZIndexes = [model1?.zIndex, model2?.zIndex, model3?.zIndex].filter(
      (z) => z !== undefined,
    ) as number[];
  ...
  it("场景3: 置底操作（模拟右键菜单）", () => {
    ...
    const allZIndexesForBottom = allNodesScene3
      .map((n) => n.zIndex)
      .filter((z) => z !== undefined);
    const minZIndex =
      allZIndexesForBottom.length > 0 ? Math.min(...allZIndexesForBottom) : 1;
  ...
  it("场景7: 完整用户流程测试", () => {
    ...
    const allZIndexesStep3 = allNodesStep3
      .map((n) => n.zIndex)
      .filter((z) => z !== undefined);
    const minZIndexStep3 =
      allZIndexesStep3.length > 0 ? Math.min(...allZIndexesStep3) : 1;
    ...
    console.log("验证结果:");
    console.log(
      `  - 最底层: ${sortedNodes[0].id} (zIndex: ${sortedNodes[0].zIndex})`,
    );
    console.log(
      `  - 最顶层: ${sortedNodes[sortedNodes.length - 1].id} (zIndex: ${sortedNodes[sortedNodes.length - 1].zIndex})`,
    );
  });
  ...
  it("场景9: 边界情况 - 最底层节点继续置底", () => {
    ...
    const allZIndexesScene9 = allNodesScene9
      .map((n) => n.zIndex)
      .filter((z) => z !== undefined);
    const minZIndexScene9 =
      allZIndexesScene9.length > 0 ? Math.min(...allZIndexesScene9) : 1;
