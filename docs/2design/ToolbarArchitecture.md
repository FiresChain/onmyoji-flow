...
> 当前实现已完成 Feature-module Phase 8。本文顶部描述现行边界；后半部分保留
...
Phase 7 已删除旧 toolbar 过渡 facade。当前由
`shells/common/EditorToolbar.vue` 负责：
...
- 向 standalone shell 发出更新日志与反馈命令。

`StandaloneEditorShell` 组装 `EditorToolbar`、`AboutDialogs`、文件 tabs、palette、
`FlowEditor` 与 `EditorDialogHost`；`EmbedEditorShell` 以 `isEmbed` 模式组装同一
toolbar composition，不持久化 locale，也不显示 app-only 帮助 UI。`App.vue` 与
`YysEditorEmbed.vue` 均为薄 facade。

Feature dialog 状态、repository、订阅和业务 UI 不进入 toolbar composition：
...
- 不改业务语义。
- 不新增对外 API。
- `EditorCommandBar` 只发事件；`EditorToolbar` 只做实例桥接、workspace service 调用
  和 feature host 编排。
- dialog host 持有自身 dialog/draft/repository 状态；listener 或 subscription 的
  disposer 由实际 mount 它的模块持有，不把这些状态回流到 toolbar composition。
- App、Embed、command bar 与 store 不直接调用 LogicFlow API。

## 兼容性

Phase 7 保留现有交互、公开 Embed API、RootDocument 和 team-code 导入。以下 key 不变：
...
standalone/embed shell 与根 facade 已完成。目录依赖规则会阻止 command bar/shell
绕过 feature public boundary 或直接导入 LogicFlow SDK；knip 与完整 CI gate 已启用。

## Phase 2 历史记录

以下 task/path 及其中的 `Toolbar.vue` 名称描述的是 Phase 2 中间态，已被上述 feature
hosts、command bar、`EditorToolbar` 与 shells 取代，不再代表当前生产路径。
...
46. Toolbar 导入模板局部不变量再收紧：结构相邻性 + 接线唯一性 AST 守卫：`toolbar-architecture.guard`
47. ImportExport 阈值前 flush 变体矩阵（三次）+ 批次隔离补强：重复 pre-threshold flush 循环与多批次分段确定性守卫：`useToolbarImportExportCommands`
48. Toolbar 导入弹窗结构锚点再补强：命令顺序漂移 + class 漂移噪声矩阵下作用域唯一命中与多轮计数对齐守卫：`toolbar-wiring.regression`
49. Toolbar 导入模板局部不变量再收紧：表单归属 + footer 相邻互斥 + 二维码接线唯一 AST 守卫：`toolbar-architecture.guard`
50. ImportExport 计时确定性再补强：重复 pre-threshold flush 循环 + 即时 rebatch 隔离下分段推进与零残留定时器守卫：`useToolbarImportExportCommands`
51. Toolbar 导入弹窗结构锚点再补强：footer 分支顺序漂移 + 同 class 假锚点组合噪声下作用域唯一命中与多轮计数对齐守卫：`toolbar-wiring.regression`
52. Toolbar 导入模板局部不变量再收紧：表单归属 + footer 顺序互斥 + 二维码接线唯一 AST 守卫：`toolbar-architecture.guard`
53. ImportExport 计时确定性再补强：重复 pre-threshold flush + 链式即时 rebatch 隔离下分段推进与零残留定时器守卫：`useToolbarImportExportCommands`
54. Toolbar 导入弹窗结构锚点再补强：slot/footer 组合漂移 + 同文案同属性假动作噪声矩阵下作用域唯一命中与多轮计数对齐守卫：`toolbar-wiring.regression`
55. Toolbar 导入模板局部不变量再收紧：slot-footer 归属 + 分支顺序互斥 + 接线配对 AST 守卫：`toolbar-architecture.guard`
56. ImportExport 计时确定性再补强：混合 flush 窗口 + 链式即时 rebatch 隔离下分段推进与零残留定时器守卫：`useToolbarImportExportCommands`
57. Toolbar 导入弹窗结构锚点再补强：slot/footer 组合漂移 + class 漂移 + footer 分支顺序漂移 + 同文案同属性假动作噪声矩阵下作用域唯一命中与多轮计数对齐守卫：`toolbar-wiring.regression`
...
64. Toolbar 导入模板局部不变量再收紧：slot-footer 归属 + 分支互斥 + event-uniqueness/action-pair AST 守卫（二次）：`toolbar-architecture.guard`
65. ImportExport 计时确定性再补强：交替双窗口混合 flush + 重复 pre-threshold loops + 链式即时 rebatch 零残留循环（二次矩阵）：`useToolbarImportExportCommands`
...
## Task 46 落地（Toolbar 导入模板局部不变量：结构相邻性 + 接线唯一性 AST 守卫）

增强：`src/__tests__/toolbar-architecture.guard.test.ts`

回归目标：

- 锁定 `import-form` / `importSource` / `teamCodeInput` 在全模板与导入弹窗局部模板中均保持唯一，且局部绑定归属导入表单（不漂移到 form 外）。
- 锁定 footer 三按钮结构相邻且互斥稳定：关闭按钮 + `json(v-if)` + `teamCode(v-else)`，显式禁止 `v-else-if` 漂移。
- 锁定 `team-code-qr-actions` 与二维码 input 接线唯一保持 `ref="teamCodeQrInputRef" + @change="handleTeamCodeQrImport" + accept="image/*"`。
- 通过 AST 守卫继续约束 `Toolbar.vue` 为接线层：`useToolbarImportExportCommands` 入参键完整，且不回流 `teamCodeService` 与 import/export 实现依赖。

## Task 47 落地（ImportExport 阈值前 flush 变体矩阵（三次）+ 批次隔离）

增强：`src/__tests__/useToolbarImportExportCommands.test.ts`

回归目标：

- 在 3 组交错变体中，连续执行两轮 `99ms` 阈值前 `runOnlyPendingTimers`（repeated pre-threshold flush loops），持续锁定 `preview/export/updateTab` 计数无漂移。
- 每组在 pre-threshold flush 收束后触发新批次，并按 `99ms -> 1ms -> 1899ms -> 1ms` 分段推进，验证 `preview/export` 无提前触发且批次隔离稳定。
- 在每次 flush 后、每组批次收束后、全流程结束后重复断言 `vi.getTimerCount() === 0`，确保无残留定时器泄漏。

## Task 48 落地（Toolbar 导入弹窗结构锚点：命令顺序漂移 + class 漂移噪声矩阵）

增强：`src/__tests__/toolbar-wiring.regression.test.ts`

回归目标：

- 在非导入弹窗注入同 class 假结构（`import-form` / `dialog-footer` / `team-code-qr-actions`）与同文案假按钮（`选择 JSON 文件` / `导入阵容码` / `选择二维码图片`）并叠加额外噪声按钮、命令顺序漂移时，导入弹窗作用域仍唯一命中真实结构锚点。
- 在 `teamCode` 来源下，真实 `team-code-qr-actions` 继续仅归属导入弹窗作用域；即使存在同 `accept="image/*"` 的假 actions，也不被误命中。
- 在 command-order + class-drift 噪声矩阵的多轮“打开/切换来源/关闭/重开（含关闭后污染态）”循环中，`openImportDialog + json/teamCode/qr` 三路径命令计数持续对齐。

## Task 49 落地（Toolbar 导入模板局部不变量：表单归属 + footer 相邻互斥 AST 守卫）

增强：`src/__tests__/toolbar-architecture.guard.test.ts`

回归目标：

- 锁定 `import-form` / `importSource` / `teamCodeInput` 在全模板与导入弹窗局部保持唯一，且 `importSource/teamCodeInput` 绑定严格归属导入表单，不漂移到 form 外或导入弹窗外。
- 锁定 footer 三按钮结构相邻稳定：关闭按钮 + `json(v-if)` + `teamCode(v-else)`，并显式禁止 `v-else-if` 漂移。
- 锁定 `team-code-qr-actions` 与二维码 input 接线唯一保持 `ref="teamCodeQrInputRef" + @change="handleTeamCodeQrImport" + accept="image/*"`。
- 通过 AST 守卫继续约束 `Toolbar.vue` 仅接线层：不回流 `teamCodeService` 与 import/export 实现依赖，且 `useToolbarImportExportCommands` 入参键完整。

## Task 50 落地（ImportExport 重复阈值前 flush 循环 + 即时新批次隔离）

增强：`src/__tests__/useToolbarImportExportCommands.test.ts`

回归目标：

- 在 3 组以上交错变体中，前两批均在 `99ms` 阈值前执行 `runOnlyPendingTimers`，持续锁定 `preview/export/updateTab` 计数无漂移。
- 每组在 pre-threshold flush 后立即触发新批次，并按 `99ms -> 1ms -> 1899ms -> 1ms` 分段推进，持续验证 `preview/export` 无提前触发。
- 在每次 flush 后、每组批次收束后、全流程结束后均断言 `vi.getTimerCount() === 0`，确保无残留定时器泄漏。

## Task 51 落地（Toolbar 导入弹窗结构锚点：footer 顺序漂移 + 同 class 假锚点组合噪声）

增强：`src/__tests__/toolbar-wiring.regression.test.ts`

回归目标：

- 在非导入弹窗注入同文案假按钮（`选择 JSON 文件` / `导入阵容码` / `选择二维码图片`）+ 假 `import-form`/`team-code-qr-actions`，并叠加 footer 分支顺序漂移与额外噪声按钮时，导入弹窗作用域仍唯一命中真实结构锚点。
- 在 `teamCode` 来源下，真实 `team-code-qr-actions` 继续仅归属导入弹窗作用域，含 `accept="image/*"` 的假 actions 不被误命中。
- 在 footer-order-drift same-class fake-anchor 噪声矩阵的多轮“打开/切换来源/关闭/重开（含关闭后污染态）”循环中，`openImportDialog + json/teamCode/qr` 三路径命令计数持续对齐。

## Task 52 落地（Toolbar 导入模板局部不变量：表单归属 + footer 顺序互斥 AST 守卫）

增强：`src/__tests__/toolbar-architecture.guard.test.ts`

回归目标：

- 锁定 `import-form` / `importSource` / `teamCodeInput` 在全模板与导入弹窗局部保持唯一，且局部绑定严格归属导入表单，不漂移到 form 外或导入弹窗外。
- 锁定 footer 三按钮顺序稳定：关闭按钮 + `json(v-if)` + `teamCode(v-else)`，显式禁止分支互换与 `v-else-if` 漂移。
- 锁定 `team-code-qr-actions` 与二维码 input 接线唯一保持 `ref="teamCodeQrInputRef" + @change="handleTeamCodeQrImport" + accept="image/*"`。
- 通过 AST 守卫继续约束 `Toolbar.vue` 为接线层，不回流 `teamCodeService` 与 import/export 实现依赖，且 `useToolbarImportExportCommands` 入参键完整。

## Task 53 落地（ImportExport 重复 pre-threshold flush + 链式即时 rebatch 隔离）

增强：`src/__tests__/useToolbarImportExportCommands.test.ts`

回归目标：

- 在 4 组以上交错变体中，前两批都在 `99ms` 阈值前执行 `runOnlyPendingTimers`，持续锁定 `preview/export/updateTab` 计数无漂移。
- 每次 flush 后立即触发新批次（至少两次链式 rebatch），并对后续批次按 `99ms -> 1ms -> 1899ms -> 1ms` 分段推进，验证无提前触发。
- 在每次 flush 后、每组批次收束后、全流程结束后均断言 `vi.getTimerCount() === 0`，确保无残留定时器泄漏。

## Task 54 落地（Toolbar 导入弹窗结构锚点：slot/footer 组合漂移 + 同文案同属性假动作噪声矩阵）

增强：`src/__tests__/toolbar-wiring.regression.test.ts`

回归目标：

- 在非导入弹窗注入同文案假按钮（`选择 JSON 文件` / `导入阵容码` / `选择二维码图片`）+ 假 `import-form` / 假 `team-code-qr-actions`（含 `accept="image/*"`）并叠加 slot/footer 组合漂移时，导入弹窗作用域仍唯一命中真实结构锚点。
- 在 `teamCode` 来源下，真实 `team-code-qr-actions` 继续仅归属导入弹窗作用域；同属性假 actions 不被误命中。
- 在 slot-footer drift + same-attr fake-action 噪声矩阵的多轮“打开/切换来源/关闭/重开（含关闭后污染态）”循环中，`openImportDialog + json/teamCode/qr` 三路径命令计数持续对齐。

## Task 55 落地（Toolbar 导入模板局部不变量：slot-footer 归属 + 分支顺序互斥 + 接线配对 AST 守卫）

增强：`src/__tests__/toolbar-architecture.guard.test.ts`

回归目标：

- 锁定 `import-form` / `importSource` / `teamCodeInput` 在全模板与导入弹窗局部保持唯一，且局部绑定继续严格归属导入表单，不漂移到 form 外或导入弹窗外。
- 锁定 footer 三按钮结构顺序稳定：关闭按钮 + `json(v-if)` + `teamCode(v-else)`，显式禁止分支互换与 `v-else-if` 漂移。
- 锁定 `team-code-qr-actions` 与二维码 input 接线保持唯一（`ref + @change + accept="image/*"`），并新增“按钮/输入配对存在但不依赖固定先后顺序”守卫。
- 通过 AST 守卫继续约束 `Toolbar.vue` 为接线层，不回流 `teamCodeService` 与 import/export 实现依赖，且 `useToolbarImportExportCommands` 入参键保持完整。

## Task 56 落地（ImportExport 混合 flush 窗口 + 链式即时 rebatch 隔离）
...
- 在 4 组以上交错变体中，前两批都在 `99ms` 阈值前执行 `runOnlyPendingTimers`，持续锁定 `preview/export/updateTab` 计数无漂移。
- 每次 flush 后立即触发新批次（至少两次链式 rebatch），并将后续批次按 `99ms -> 1ms -> 1899ms -> 1ms` 分段推进，持续验证无提前触发。
- 在每次 flush 后、每组批次收束后、全流程结束后均断言 `vi.getTimerCount() === 0`，确保无残留定时器泄漏。

## Task 57 落地（Toolbar 导入弹窗结构锚点：复合漂移噪声矩阵）

增强：`src/__tests__/toolbar-wiring.regression.test.ts`

回归目标：

- 在非导入弹窗注入假 `import-form` / 假 `dialog-footer` / 假 `team-code-qr-actions` 多层噪声锚点，并叠加 slot/footer 组合漂移、class 漂移、footer 分支顺序漂移与额外噪声按钮时，导入弹窗作用域仍唯一命中真实结构锚点。
- 在 `teamCode` 来源下，真实 `team-code-qr-actions` 继续仅归属导入弹窗作用域；含同文案按钮与 `accept="image/*"` 的假 actions 不被误命中。
- 在复合漂移噪声矩阵的多轮“打开/切换来源/关闭/重开（含关闭后污染态）”循环中，`openImportDialog + json/teamCode/qr` 三路径命令计数持续对齐。

## Task 58 落地（Toolbar 导入模板局部不变量：slot-footer 归属 + 分支互斥 + action-pair AST）

增强：`src/__tests__/toolbar-architecture.guard.test.ts`

回归目标：

- 锁定 `import-form` / `importSource` / `teamCodeInput` 在全模板与导入弹窗局部保持唯一，且绑定继续归属导入表单，不漂移到 form 外或导入弹窗外。
- 锁定 footer 三按钮顺序稳定：关闭按钮 + `json(v-if)` + `teamCode(v-else)`，禁止分支互换与 `v-else-if` 漂移。
- 锁定 `team-code-qr-actions` 与二维码 input 接线唯一（`ref + @change + accept="image/*"`），并新增 action/input 配对存在性守卫且不依赖固定先后顺序。
- 通过 AST 守卫继续约束 `Toolbar.vue` 为接线层，不回流 `teamCodeService` 与 import/export 实现依赖，`useToolbarImportExportCommands` 入参键保持完整。

## Task 59 落地（ImportExport 双窗口混合 flush + 链式 rebatch 零残留循环）

增强：`src/__tests__/useToolbarImportExportCommands.test.ts`

回归目标：

- 在 4 组以上交错变体中，前两批均在 `99ms` 阈值前执行 `runOnlyPendingTimers`，持续锁定 `preview/export/updateTab` 计数无漂移。
- 每次 flush 后立即触发新批次（至少两次链式 rebatch），并对后续两批按 `99ms -> 1ms -> 1899ms -> 1ms` 分段推进，持续验证无提前触发。
- 在每次 flush 后、每组批次收束后、全流程结束后均断言 `vi.getTimerCount() === 0`，确保无残留定时器泄漏。

## Task 60 落地（Toolbar 导入弹窗结构锚点：嵌套 slot-footer 次级顺序 + duplicate-accept 假动作噪声）
...
## Task 64 落地（Toolbar 导入模板局部不变量：slot-footer 归属 + 分支互斥 + event-uniqueness/action-pair AST 守卫二次收紧）

增强：`src/__tests__/toolbar-architecture.guard.test.ts`

回归目标：

- 锁定 `import-form` / `importSource` / `teamCodeInput` 在全模板、导入弹窗局部、导入表单局部持续唯一，且绑定归属导入表单，不漂移到 form 外或导入弹窗外。
- 锁定导入 footer 三按钮顺序稳定：关闭按钮 + `json(v-if)` + `teamCode(v-else)`，显式禁止分支互换与 `v-else-if` 漂移。
- 强化 event-uniqueness：`triggerJsonFileImport` 与 `handleTeamCodeImport` 在导入 footer 分支中保持唯一绑定，且不漂移到 footer 外。
- 锁定 `team-code-qr-actions` 与二维码 input 接线唯一（`ref + @change + accept="image/*"`），并持续守卫 action/input 配对存在且不依赖固定先后顺序。
- 持续约束 `Toolbar.vue` 为接线层，不回流 `teamCodeService` 与 import/export 实现依赖，`useToolbarImportExportCommands` 入参键保持完整。

## Task 65 落地（ImportExport 交替双窗口混合 flush + 重复 pre-threshold loops + 链式即时 rebatch 零残留循环二次矩阵）
...
## Task 69 落地（Toolbar 导入弹窗结构锚点：slot/footer 分支顺序漂移 + class 漂移 + duplicate-accept hidden-input 扩展噪声矩阵 v3）
