...
默认值事实源为 `src/core/document/types.ts:DefaultNodeStyle`：
...
## 归一化与工具（`src/core/document/nodeStyle.ts` 与 `src/editor/nodeStyle.ts`）
...
## 复用方案：`useNodeAppearance`（`src/editor/node-types/useNodeAppearance.ts`）
